...
func RecvPacket(conn net.Conn) (int, int, HeaderValues, []byte, error) {
	// receive
	buffer := make([]byte, 2+3+4+(16*1024*1024)+32+3+(16*1024*1024)+32)
	...
	headerRaw := buffer[2+3+4 : 2+3+4+headerLength]
	log.Println("header:", hex.EncodeToString(headerRaw))

	headerChecksum := buffer[2+3+4+headerLength : 2+3+4+headerLength+32]
	log.Println("header checksum:", hex.EncodeToString(headerChecksum))

	bodyLength := binary.BigEndian.Uint32(buffer[2+3+4+headerLength+32 : 2+3+4+headerLength+32+3])
	log.Println("body length:", bodyLength)

	body := buffer[2+3+4+headerLength+32+3 : 2+3+4+headerLength+32+3+bodyLength]
	log.Println("body:", hex.EncodeToString(body))

	bodyChecksum := buffer[2+3+4+headerLength+32+3+bodyLength : 2+3+4+headerLength+32+3+bodyLength+32]

...
func DeleteFile(path string) (HeaderMetadata, error) {
	...
	if !strings.HasPrefix(path, PUBLIC_FOLDER) {
		return retError(metadata, ERROR_ACCESS_DENIED, "You are not permitted to delete this file"), &ErrAccessDenied{}
	}

	stat, err := os.Stat(path)
	if err != nil {
		if os.IsNotExist(err) {
			return retError(metadata, ERROR_FILE_NOT_EXISTS, "File does not exist"), err
		}
	...
	err = os.Remove(path)
	if err != nil {
		return retError(metadata, ERROR_UNKNOWN, "Cannot delete file"), err
	}
